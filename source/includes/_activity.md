...
The individual SLA metrics are a breakdown at an individual level of the
performance of the team on specific objects. This report can take some time to
generate, so the output is generated asynchronously. The endpoint will
initially return a result indicating that the request has been accepted and the
report is generated. The acceptance response contains a report identifier token
that can be used to check the current processing status of the report.

```text
curl https://api.lexer.io/v1/activity/reports \
-H "Content-Type: application/json" \
-H "Auth-Api-Token: 12345678-1234-1234-1234-123456789" \
-d '
{
  "type": "mention_sla_export",
  "date_from": "2018-10-01T00%3A00Z",
  "date_to": "2018-11-15T00%3A00Z"
}
'
```

Note that the completed report generated by this endpoint will export data in
CSV format, and will contain the following columns in the generated report:

| Column                      | Description                                              | Type   |
| --------------------------- | -------------------------------------------------------- | ------ |
| Account Name                | name of the account mentioned                            | string |
| Source Type                 | source of the mention                                    | string |
| Agent First Name            | first name of handling agent                             | string |
| Agent Last Name             | last name of handling agent                              | string |
| Last Action Date (UTC)      | time of last action                                      | string |
| Customer Wait Time (min)    | time customer spent waiting for response                 | string |
| In Queue (min)              | time mention spent in queue                              | string |
| Agent Response Time (min)   | time taken for agent to respond after claiming ownership | string |
| Within Customer SLA         |                                                          |
| Within Agent SLA            |                                                          |
| State                       | the current state of the object                          | string |
| Customer Publish Date (UTC) | the publish date of the object                           | string |
| Customer Username           | username of the customer                                 | string |
| Customer Fullname           | full name of the customer                                | string |
| Customer Message            | customer's object message                                | string |
| Agent Reponse Date (UTC)    | the response date from the agent                         | string |
| Agent Response              | agent's response message                                 | string |
| Classifications             | classifications applied to the object                    | string |
| Agent Notes                 | notes added to the case                                  | string |
| Customer Note               | notes added to the customer's identity                   | string |
...
| Property  | Description      | Type   |
| --------- | ---------------- | ------ |
| date_from | from date in UTC | string |
| date_to   | to date in UTC   | string |
...
`https://api.lexer.io/v1/activity/reports/status`

If a report returns an entity witha `status: "pending"` property, the report result is being processed asynchronously. The status endpoint can be used to interrogate the status of a running report.

```text
curl https://api.lexer.io/v1/activity/reports?status=12345678-1234-1234-1234-123456789 \
-H "Auth-Api-Token: 12345678-1234-1234-1234-123456789" \
```
...
{
  "status": "complete",
  "url": "https://lexer-dashboard-exports.s3.ap-southeast-2.amazonaws.com/activity_12345678-1234-1234-1234-123456789",
  "token": "12345678-1234-1234-1234-123456789"
}
```
